...
class UniversalModel extends CI_Model
{
	 ...
	 * @return array
	 */
	public function getAllData(string $namaTable, string $where = "", string $selectColumn = "*" ,  string $orderBy = "", string $orderByOption = "DESC")
	{
	...
	public function getMetaLink(string $table = "bab", string $title = "kursus")
	{

		$slug = preg_replace("/-$/", "", preg_replace('/[^a-z0-9]+/i', "-", strtolower($title)));

		$query = $this->db
			->where("meta_link_" . $table . " like", "{$slug}%")
			->get($table)
			->result_array();

		$numHits = count($query);

		$link = ($numHits > 0) ? ($slug . '-' . $numHits) : $slug;
