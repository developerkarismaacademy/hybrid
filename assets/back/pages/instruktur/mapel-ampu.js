var ListMapelAmpu = function () {

	//INIT URL
	var urlGetApi = base_url_api + "/ProgressApi/jsondatamapel";
	...
	var noImg = base_url + "upload/mapel/default.jpg"
	...
	//INIT GET DATA
	var idInstruktur = $("#idInstruktur").val();
	var dataMapelAmpu = [];
	var totalData = 0;
	...
	var requestMapelAmpu = function (url) {
		if (!loading) {
			loading = true;
			url = url + "?page=" + page;
			url = url + "&search=" + search;
			url = url + "&limit=" + limit;
			url = url + "&idInstruktur=" + idInstruktur;
	...
	var updateListMapelAmpu = function () {
		$('.data-mapel').remove();
		$.each(dataMapelAmpu, function (index, value) {
			...
			var nama_mapel = value.nama_mapel;
			nama_mapel = "<span title='" + value.nama_mapel + "'>" + nama_mapel + "</span>";

			clone.find('.nama-mapel').html(nama_mapel);

			clone.find('.gambar-mapel').prop("src", noImg);
			if (!ListMapelAmpu.UrlExists(base_url + "/upload/banner_mapel/" + value.banner_mapel) && value.banner_mapel != "") {
				clone.find('.gambar-mapel').prop("src", base_url + "/upload/banner_mapel/" + value.banner_mapel);
			} else if (!ListMapelAmpu.UrlExists(base_url + "/upload/mapel/" + value.gambar_mapel) && value.gambar_mapel != "") {
				clone.find('.gambar-mapel').prop("src", base_url + "/upload/mapel/" + value.gambar_mapel);
			}

			if (value.jml_notif !== null) {
				if (parseInt(value.jml_notif) > 10) {
					clone.find('.jml-nilai').html("10+");
				} else {
					clone.find('.jml-nilai').html(value.jml_notif);
				}
			} else {
				clone.find('.jml-nilai').hide();
			}

			clone.find('.link-detail').prop('href', base_url + 'back/siswa-ampu/' + value.meta_link_mapel);
			clone.find('.link-detail').prop('title', value.nama_mapel);
	...
	var setUpPagination = function (data) {
		...
		if ((page - 1) <= 0) {
			...
			btnPagePreviousElement.find('.page-link').prop('href', 'javascript:ListMapelAmpu.changePage(' + (page - 1) + ')');
		...
		if ((page + 1) > totalPage) {
			...
			btnPageNextElement.find('.page-link').prop('href', 'javascript:ListMapelAmpu.changePage(' + (page + 1) + ')');
		...
		if (page == 1) {
			...
			cloneBtnPage.find('.page-link').prop('href', 'javascript:ListMapelAmpu.changePage(1)');
...
// console.log(start);
		...
		for (var i = start; i < totalPage; i++) {
			...
			if (page == i) {
				...
				cloneBtnPage.find('.page-link').prop('href', 'javascript:ListMapelAmpu.changePage(' + i + ')');
				cloneBtnPage.removeClass('disabled');
				cloneBtnPage.removeClass('active');
			}
		...
		if (totalPage > 1) {
			...
			if (page == totalPage) {
				...
				cloneBtnPage.find('.page-link').prop('href', 'javascript:ListMapelAmpu.changePage(' + totalPage + ')');
				cloneBtnPage.removeClass('disabled');
				cloneBtnPage.removeClass('active');
			}
