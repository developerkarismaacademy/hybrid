var ListIndikatorInduk = function () {

	//INIT URL
	var urlGetApi = base_url_api + "/IndikatorIndukApi/jsondata";
	var urlDetailApi = base_url_api + "/IndikatorIndukApi/detail/";
	var urlDeleteApi = base_url_api + "/IndikatorIndukApi/delete/";
	...
	var requestIndikatorInduk = function (url) {
		if (!loading) {
			loading = true;
			url = url + "?page=" + page;
	...
	var updateListIndikatorInduk = function () {
		$('.data-indikator-induk').remove();
		$.each(dataIndikatorInduk, function (index, value) {
			var clone = cloneDataElement.clone();

			clone.addClass("data-indikator-induk");
			...
			clone.find('.link-detail').prop('href', 'javascript:ListIndikatorInduk.detailIndikatorInduk(' + value.id_indikator_induk + ');');
			clone.find('.link-daftar').prop('href', base_url + "back/indikator/" + value.id_indikator_induk);
			clone.find('.link-hapus').prop('href', 'javascript:ListIndikatorInduk.deleteIndikatorInduk(' + value.id_indikator_induk + ');');
			clone.find('.link-ubah').prop('href', base_url + "back/indikator-induk/" + idKompetensi + "/ubah/" + value.id_indikator_induk);
	...
	var setUpPagination = function (data) {
		...
		if ((page - 1) <= 0) {
			...
			btnPagePreviousElement.find('.page-link').prop('href', 'javascript:ListIndikatorInduk.changePage(' + (page - 1) + ')');
		...
		if ((page + 1) > totalPage) {
			...
			btnPageNextElement.find('.page-link').prop('href', 'javascript:ListIndikatorInduk.changePage(' + (page + 1) + ')');
		...
		if (page == 1) {
			...
			cloneBtnPage.find('.page-link').prop('href', 'javascript:ListIndikatorInduk.changePage(1)');
...
// console.log(start);
		...
		for (var i = start; i < totalPage; i++) {
			...
			if (page == i) {
				...
				cloneBtnPage.find('.page-link').prop('href', 'javascript:ListIndikatorInduk.changePage(' + i + ')');
				cloneBtnPage.removeClass('disabled');
				cloneBtnPage.removeClass('active');
			}
		...
		if (totalPage > 1) {
			...
			if (page == totalPage) {
				...
				cloneBtnPage.find('.page-link').prop('href', 'javascript:ListIndikatorInduk.changePage(' + totalPage + ')');
				cloneBtnPage.removeClass('disabled');
				cloneBtnPage.removeClass('active');
			}
	...
	var deleteIndikatorInduk = function (id) {
		if (!loading) {
			loadingElement.show();
			tableDateElement.hide();
			loading = true;
			jQuery.getJSON(urlDetailApi + id, result => {
				...
				loading = false;
				swal({
					title: "Apa Anda Yakin Ingin Menghapus Indikator Induk " + nama + "?",
	...
	var deleteServer = function (id) {
		if (!loading) {
			...
			jQuery.getJSON(urlDeleteApi + id, result => {
				...
				loadingElement.hide();
				tableDateElement.show();
				requestIndikatorInduk(urlGetApi);
			})
		}
	};
	...
	return {
		...
		detailIndikatorInduk(id) {
			detailOpen(id);
		},
		deleteIndikatorInduk(id) {
			deleteIndikatorInduk(id);
		},
		modalClose() {
