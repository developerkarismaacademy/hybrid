var FormBab = function () {
	...
	var nama_bab, kelas_id, mapel_id, deskripsi_bab, gambar_bab, pretest_status;
	...
	var requestGetMapel = function () {
		if (!loading) {
			...
			request.done(function (xhr, status, error) {
				...
				mapelElement.val(id).trigger('change');
				loading = false;
		...
		if (!loading) {
			...
			request.done(function (xhr, status, error) {
				...
				if (xhr.data.gambar_bab != "") {
					...
					imgGambarBabElement.attr('src', base_url + "/upload/mapel/" + xhr.data.gambar_bab);
