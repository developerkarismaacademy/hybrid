var FormUpdateVideo = function () {
	...
	var urlVideoElement = $('#url_video');
	var namaMateriElement = $('#nama_materi');
	var isiMateriElement = $('#isi_materi');
	var deskripsiMateriElement = $('#deskripsi_materi');
	var durasiMateriMenitElement = $('#durasi_materi_menit');
	var durasiMateriDetikElement = $('#durasi_materi_detik');
	var urlSimpanApi = base_url_api + "/MateriApi/ubahVideo";
	...
	var nama_materi, kelas_id, mapel_id, bab_id, url_video, isi_materi, deskripsi_materi, durasi_materi;

	//MODAL
	var loadingModalElement = $('#loading-modal');
	var modalVideoElement = $('#modal-video');
	var modalContainerElement = $('#modal-container');
	var videoElement = $('#video');
	var videoThumbnailElement = $('#video-thumbnail');
	...
	var initValidation = function () {
		formMateriElement.validate({
			...
			messages: {
				...
				'url_video': {
					required: 'ID Youtube Video Harus Di Isi',
	...
	urlVideoElement.change(function () {
		loadVideo();
	});

	urlVideoElement.keyup(function () {
		loadVideo();
	});

	var loadVideo = function () {
		var urlVid = urlVideoElement.val();
		videoElement.attr('src', "https://www.youtube.com/embed/" + urlVid + "?autoplay=1&mute=1");
	};
	...
	var detailOpen = function () {
		if (!loading) {
			...
			request.done(function (xhr, status, error) {
				...
				durasi = xhr.data.durasi.split(".");
				isi_materi = xhr.data.isi_materi;
				url_video = xhr.data.url_video;
				...
				urlVideoElement.val(url_video);
				durasiMateriMenitElement.val(durasi[0]);
				durasiMateriDetikElement.val(durasi[1]);
	...
	return {
		...
		checkVideo: function () {
			if (urlVideoElement.val() != "") {
				loadVideo();
				modalVideoElement.modal('toggle');
			} else {
				toastr['error']('Video masih kosong', 'Silahkan dicek kembali');
			}
		},
		modalClose() {
