var ListTestimoni = function () {

	//INIT URL
	var urlGetApi = base_url_api + "/TestimoniApi/jsondata";
	var urlDetailApi = base_url_api + "/TestimoniApi/detail/";
	var urlDeleteApi = base_url_api + "/TestimoniApi/delete/";
	var id = $('#idMapel').val();
	...
	var updateListRating = function () {
		...
		$.each(dataRating, function (index, value) {
			...
			clone.find('.nama_user').html(value.nama_user);
			clone.find('.rating').html(value.rating);
			clone.find('.ulasan').html(value.ulasan);


			clone.find('.link-detail').prop('href', 'javascript:ListTestimoni.detailRating(' + value.user_id + ');');
			clone.find('.link-hapus').prop('href', 'javascript:ListTestimoni.deleteRating(' + value.user_id + ');');
			clone.find('.link-ubah').prop('href', base_url + "back/rating/" + value.meta_link_rating + "/ubah");
	...
	var setUpPagination = function (data) {
		...
		if ((page - 1) <= 0) {
			...
			btnPagePreviousElement.find('.page-link').prop('href', 'javascript:ListTestimoni.changePage(' + (page - 1) + ')');
		...
		if ((page + 1) > totalPage) {
			...
			btnPageNextElement.find('.page-link').prop('href', 'javascript:ListTestimoni.changePage(' + (page + 1) + ')');
		...
		if (page == 1) {
			...
			cloneBtnPage.find('.page-link').prop('href', 'javascript:ListTestimoni.changePage(1)');
...
// console.log(start);
		...
		for (var i = start; i < totalPage; i++) {
			...
			if (page == i) {
				...
				cloneBtnPage.find('.page-link').prop('href', 'javascript:ListTestimoni.changePage(' + i + ')');
				cloneBtnPage.removeClass('disabled');
				cloneBtnPage.removeClass('active');
			}
		...
		if (totalPage > 1) {
			...
			if (page == totalPage) {
				...
				cloneBtnPage.find('.page-link').prop('href', 'javascript:ListTestimoni.changePage(' + totalPage + ')');
				cloneBtnPage.removeClass('disabled');
				cloneBtnPage.removeClass('active');
			}
	...
	var detailOpen = function (id) {
		...
		if (!loading) {
			...
			request.done(function (xhr, status, error) {
				...
				emailElement.html(xhr.data.email_user);
