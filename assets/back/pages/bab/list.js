var ListBab = function () {

	//INIT URL
	var urlGetApi = base_url_api + "/BabApi/jsondata";
	var urlDetailApi = base_url_api + "/BabApi/detail/";
	var urlDeleteApi = base_url_api + "/BabApi/delete/";
	...
	var deskripsiBabDetailElement = $('#deskripsi-bab-detail');
	var statusBabDetailElement = $('#status-bab-detail');
	var imgGambarBabDetailElement = $('#img-gambar-bab-detail');

	var requestBab = function (url) {
		if (!loading) {
			loading = true;
			url = url + "?page=" + page;
	...
	var updateListBab = function () {
		$('.data-bab').remove();
		$.each(dataBab, function (index, value) {
			var clone = cloneDataElement.clone();

			clone.addClass("data-bab");
			clone.prop('id', 'data' + (index + 1));
			clone.removeClass("clone-data");

			clone.find('.nama-bab').html(value.nama_bab);

			if (value.pretest_status == 1 || value.posttest_status == 1) {
				if (value.pretest_status == 1) {

					clone.find('.status-bab').html('<span class="label label-info">Pretest</span>');
				} else {
					clone.find('.status-bab').html('<span class="label label-info">Post-test</span>');

				}
			} else {
				clone.find('.status-bab').html('<span class="label label-primary">Premium</span>');
			}


			clone.find('.link-detail').prop('href', 'javascript:ListBab.detailBab(' + value.id_bab + ');');
			clone.find('.link-hapus').prop('href', 'javascript:ListBab.deleteBab(' + value.id_bab + ');');
			clone.find('.link-ubah').prop('href', base_url + "back/bab/" + value.meta_link_bab + "/ubah");
			clone.find('.link-daftar').prop('href', base_url + "back/materi/" + value.meta_link_bab);
	...
	var setUpPagination = function (data) {
		...
		if ((page - 1) <= 0) {
			...
			btnPagePreviousElement.find('.page-link').prop('href', 'javascript:ListBab.changePage(' + (page - 1) + ')');
		...
		if ((page + 1) > totalPage) {
			...
			btnPageNextElement.find('.page-link').prop('href', 'javascript:ListBab.changePage(' + (page + 1) + ')');
		...
		if (page == 1) {
			...
			cloneBtnPage.find('.page-link').prop('href', 'javascript:ListBab.changePage(1)');
		...
		for (var i = start; i < totalPage; i++) {
			...
			if (page == i) {
				...
				cloneBtnPage.find('.page-link').prop('href', 'javascript:ListBab.changePage(' + i + ')');
				cloneBtnPage.removeClass('disabled');
				cloneBtnPage.removeClass('active');
			}
		...
		if (totalPage > 1) {
			...
			if (page == totalPage) {
				...
				cloneBtnPage.find('.page-link').prop('href', 'javascript:ListBab.changePage(' + totalPage + ')');
				cloneBtnPage.removeClass('disabled');
				cloneBtnPage.removeClass('active');
			}
		...
		if (!loading) {
			...
			jQuery.getJSON(urlDetailApi + id, result => {
				...
				swal({
					...
					text: "Semua Data Bab Akan Terhapus Dan Tidak Bisa Di Kembalikan",
					type: "error",
	...
	var detailOpen = function (id) {
		...
		if (!loading) {
			...
			request.done(function (xhr, status, error) {
				namaBabDetailElement.html(xhr.data.nama_mapel);
				deskripsiBabDetailElement.html(xhr.data.deskripsi_bab);
				namaBabDetailElement.html(xhr.data.nama_bab);
				namaMapelDetailElement.html(xhr.data.nama_mapel);
				namaKelasDetailElement.html(xhr.data.nama_kelas);
				if (xhr.data.pretest_status == 1 || xhr.data.posttest_status == 1) {
					if (xhr.data.pretest_status == 1) {

						statusBabDetailElement.html('<span class="label label-info">Pretest</span>');
					} else {
						statusBabDetailElement.html('<span class="label label-info">Post-Test</span>');

					}
				} else {
					statusBabDetailElement.html('<span class="label label-primary">Premium </span>');
				}
