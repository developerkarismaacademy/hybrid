var ListAsset = function () {

	//init
	var urlGetApi = base_url_api + "/AssetApi/jsondata";
	var urlDetailApi = base_url_api + "/AssetApi/detail/";
	var urlDeleteApi = base_url_api + "/AssetApi/delete/";
	...
	var updateListAsset = function () {
		$('.data-asset').remove();
		$.each(dataAsset, function (index, value) {
			var clone = cloneDataElement.clone();

			clone.addClass("data-asset");
			clone.prop('id', 'data' + (index + 1));
			clone.removeClass("clone-data");

			clone.find('.nama').html(value.nama);
			clone.find('.link').html(value.link);
			clone.find('.tipe').html(value.tipe);

			console.log(idMateri);
			clone.find('.link-hapus').prop('href', 'javascript:ListAsset.deleteAsset(' + value.id_asset + ');');
	...
	var setUpPagination = function (data) {
		...
		if ((page - 1) <= 0) {
			...
			btnPagePreviousElement.find('.page-link').prop('href', 'javascript:ListAsset.changePage(' + (page - 1) + ')');
	...
	var deleteAsset = function (id) {
		if (!loading) {
			...
			jQuery.getJSON(urlDetailApi + id, result => {
				...
				loading = false;
				swal({
					title: "Apa Anda Yakin Ingin Menghapus Asset " + id + "?",
