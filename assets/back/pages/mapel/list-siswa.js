var ListSiswa = function () {
	...
	var updateListSiswa = function () {
		...
		$.each(dataSiswa, function (index, value) {
			...
			var transaksi_id = value.transaksi_id;
			var invoice = "#" + transaksi_id.toString().padStart(8, '0');
			...
			clone.find('.telepon-siswa').html(value.telepon_user);
			clone.find('.invoice-siswa').html(invoice);


			clone.find('.link-detail').prop('href', 'javascript:ListSiswa.detailSiswa(' + value.id_kelas + ');');
